<section id="courses" class="relative py-24 lg:py-32 bg-warm-white overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center max-w-3xl mx-auto mb-16 lg:mb-20">
            <span class="inline-block px-4 py-1.5 rounded-full bg-soft-mint-100 text-emerald-premium-700 text-sm font-bold border border-emerald-premium-200 mb-5">الدورات التعليمية</span>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach ($courses as $i => $course)
                <article class="reveal group relative bg-white rounded-[1.75rem] overflow-hidden border border-emerald-premium-100/80 shadow-card hover:shadow-premium hover:-translate-y-2 transition-all duration-500"
                         style="--reveal-delay: {{ $i * 100 }}ms">
                    <!-- Gradient header -->
                    <div class="relative h-48 bg-gradient-to-br {{ $course['gradient'] }} overflow-hidden">
                        ...
                        <div class="relative z-10 h-full flex flex-col items-center justify-center text-white p-6">
                            <svg class="w-16 h-16 opacity-90 mb-3 drop-shadow-lg" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <!-- Content -->
                    <div class="p-7">
                        <h3 class="text-2xl font-extrabold text-deep-green-800 mb-3">{{ $course['name'] }}</h3>
                        <p class="text-deep-green-600 leading-relaxed mb-6 text-[15px]">{{ $course['description'] }}</p>

                        <div class="grid grid-cols-3 gap-3 mb-7">
                            <div class="bg-soft-mint-100/70 rounded-xl p-3 text-center">
                                <p class="font-black text-deep-green-800 text-lg">{{ $course['lessons'] }}</p>
                            ...
                            <div class="bg-soft-mint-100/70 rounded-xl p-3 text-center">
                                <p class="font-black text-deep-green-800 text-lg">{{ $course['students'] }}</p>
